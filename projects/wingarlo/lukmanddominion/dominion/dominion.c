...
//Refactored cards-assignment2
int playSmithy(struct gameState *state, int currentPlayer, int handPos)
{
  int i;
  // +3 cards
  for(i = 0; i < 4; i++)		//Introduced bug i < 4, so the player would draw 4 cards, instead of i < 3
...
int playAdventurer(struct gameState *state)
{
   ...
   int z = 0;
   while(drawntreasure <= 2){			//Introduced bugs so the player draw 3 cards, instead of <2
	   if(state->deckCount[currentPlayer] < 1)
		   shuffle(currentPlayer, state); 	//if deck empty, shuffle discard pile and become the deck
	   drawCard(currentPlayer, state);
	   int cardDrawn = state->hand[currentPlayer][state->handCount[currentPlayer]-1];		//recently drawn cards
	   if(cardDrawn == copper || cardDrawn == silver || cardDrawn == gold){
		   ...
		   state->handCount[currentPlayer]--;		//remove the most recent drawn cards
		   z++;
	   }
   }
   while(z-1 >= 0){
	   state->discard[currentPlayer][state->discardCount[currentPlayer]++] = temphand[z-1];		//discard cards in play that has been drawn
	   z--;
   }
   return 0;
}
...
int playVillage(struct gameState *state, int currentPlayer, int handPos)
{
   ...
   //put played cards in discard pile
   discardCard(handPos, currentPlayer, state, 1);	//introduce bugs, where the value of trashflag is 1 instead of 0
   return 0;
}
...
int playCouncilRoom(struct gameState *state, int currentPlayer, int handPos)
{
   ...
   for(i = 0; i < state->numPlayers; i++)
	   if(i = currentPlayer)			//introduce bugs that when i = current, draw card, instead of i !=
		   drawCard(i, state);
